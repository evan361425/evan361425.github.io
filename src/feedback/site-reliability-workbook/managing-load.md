...
接著，我們就來介紹一下 GCLB 的架構，然後闡述他如何解決這些問題。
...
這樣的流量暴漲勢必考驗他們幾乎所有的技術堆棧的承載能力。
這樣的考驗也包括 GCP，在遊戲正式發佈後的兩天，該遊戲已經是 GCLB 服務中，最大的使用者了，
...
- 使用者需要到 Nginx 才能完成連線，而不是 Maglev 後馬上建立，增加處理時間，降低整體通量；
...
![因為使用者同時重傳，導致的流量高峰](concurrent-retry-spike)
...
而對 Pokémon GO 的開發者來說，他們使用調整了重傳的機制
（增加 [jitter](../../essay/web/retry-strategy.md)），
以及更有經驗的處理如何應付高流量。

## 負載管理的手段

*負載平衡*、*負載削減* 和 *自動增減* 就是我們在做負載管理時需要拿捏、權衡的三大工具。
...
例如 *區域A* 每秒送了 10 個請求之後，CPU 增加了 5%，並且閥值設在 CPU 使用率達到 80% 後就不能再給。
這代表負載平衡器可以發送每秒 160（$80/5*10$）個請求給它。
而 *區域B* 每秒送了 10 個請求後，CPU 增加到 8%，這時就只會給他 RPS 100 的量。

![區域 A 在十一點後，RPS 增加到 400，錯誤率也同步增加，其他區域的量則減少。](region-a-rps-error-increase)

早上 10:46 時因為折扣，使用者增加，RPS 從常態的 90 拉高到 120，這時正好是 11:00。
由於此時 *區域A* 的請求比較多，導致他率先突破 CPU 使用的臨界點（例如前述的 80%），
進而導致服務的負載削減觸發。
