...
-   *光譜*（spectroscopy），從牛頓開始研究光的散射後，我們發現了光是由不同顏色的光組成的。
    如今，我們可以透過觀察天文物體的光譜（藍色到紅色），推測該物體的溫度、組成、移動速度。
    ![光譜和其對應於望遠鏡的使用](https://i.imgur.com/nzX6xT0.png)
-   *影像*，有了影像，我們可以客觀的紀錄這些物體，不再是透過觀察家的口述、書寫或手繪。
    除此之外，我們也可以利用一些影像技術，獲得以前不知道的訊息，例如 CCD 可用於長期曝光，來觀察那些發光微弱的星體。
-   *多波段天文學*，光線是電磁波的一種表現形式，透過觀察不同頻率的電磁波，我們獲得除了顏色外，以往不知道的一些訊息。
    事實上，每次我們發明不同波長（例如[重力波](https://en.m.wikipedia.org/wiki/Gravitational-wave_observatory)和2008 年 NASA 發射的費米伽馬射線太空望遠鏡）的觀測手法，都會得到一些全新的發現（例如中子星噴流和[費米氣泡](https://technews.tw/2023/01/10/fermi-bubble-milky-way/)等等）。
    ![不同波段的電磁波呈現的太空一角樣貌](https://i.imgur.com/MMdvgOm.png)
-   *太空探測*，自從 1960 年代後，人類有能力在除了地球之外的地方進行觀測。
    在外太空觀測可以避免大氣層的干擾，例如 *X射線* 無法穿透大氣層。
...
-   *大小*，觀測到的物體大小會和距離成反比（注意不是平方），我們通常用角直徑來表示物體的距離和大小關係。
    一個 DVD 大小的物體，在 400 公尺遠的距離稱為 1 弧分（arch minute），在 27 公里外則稱為 1 弧秒。
...
在 [Milky Way 開放的地圖](https://djer.roe.ac.uk/vsa/vvv/mosaic/lb.html)中我們可以看到他透過多張照片的組合，整合成一個大的圖片。
主畫面的圖像只是右上角大地圖中的其中一塊，而這個大地圖又只是天空中的一小塊而已，如下圖所示。
...
因為如果用折射，為了讓成像結果變更大，我們需要讓焦距更長（所以你在中世紀看到的望遠鏡中都會很長），進而導致中間的透鏡變得很厚。

然而在反射的望遠鏡中，兩倍大的望遠鏡其成本會因為工程等等因素上升到約八倍左右（三次方成長）。
...
根據物理特性，波段越長焦距越短的光、對這種效應的影響更嚴重，以人的眼睛為例，干擾程度約為 25 弧秒。
...
-   在外太空建置望遠鏡。
-   追蹤這些晃動然後透過計算校正這個誤差。
  可以透過打雷射出去來計算這個誤差，例如歐洲極大望遠鏡將採用這方式，詳見[電腦](#_28_)。
...
又例如哈伯望眼鏡發射到太空之初，他的鏡子和理論上有大約 1 毫米的差距，進而造成成像的誤差，詳見太空任務編號 [STS-61](https://en.wikipedia.org/wiki/STS-61) 的行動。
...
X 軸由左至右為藍至紅；Y 軸由下至上為暗至亮，其中太陽位於中間偏左的位置（Luminosity = 1, Temperature = 5778K）。
...
但問題是我們怎麼取得該星體的顏色呢？
在成像前，我們在 CCD 前面裝上濾波器（類似透明色紙），只讓特定顏色的光可以打進感測器，這樣就可以收集到特定顏色的成像（例如藍色）。
反覆這種過程，就透過取得該星體在不同顏色的光亮程度。
最後整合病比較這個星體的不同顏色（通常是藍、紅、黃）的比例，來得知該星體的顏色、溫度等物理性質。

### 太空望遠鏡

如果我們想要了解星體的更多細節，我們就只能上太空建立望遠鏡，例如 *X射線* 只能在太空中觀測。
...
我們需要[很多設施](https://blogs.esa.int/luca-parmitano/)才能發射太空船：控制中心、測試機構、軟硬體等等。
最重要的是，這些東西通常需要政府支持，也就是說你會受到嚴格的預算監督。

#### 能量

在外太空你沒有電源線可以接，相對的，你需要自產能源。
通常是使用太陽能，然而太空船的太陽能版通常會很大，你需要不同的工程技術和理論。

![大片太陽能板需要可以摺疊，這樣才能裝在太空船，同時你還需要讓他能夠自動展開和配合光源（太陽）轉向](https://i.imgur.com/ezZNu5H.png)

#### 穩定

在太空中，望遠鏡若需要探測某個星體，他需要要穩定地對準它，才能獲得足夠的資訊，有幾種方式：
...
為了對準某個星體，我們需要讓望遠鏡知道自己在哪裡，並得知該星體的方向在哪裡：

-   地平線觀測器，觀看地球位置來得知自己的位置
-   太陽感測器，觀看太陽位置來得知自己的位置
-   陀螺儀
-   星體追蹤器，額外一個相機去確認目前就是對準某個星體，這相對於其他方法會比較精準

#### 輻射和溫度差

地磁會把太陽磁暴匯集在[*范艾倫輻射帶*](https://zh.wikipedia.org/zh-tw/范艾伦辐射带)（Van Allen radiation belt）上，所以需要讓太空船移動到輻射帶之外的地方。

在外太空有無照射到太陽，會讓溫度差距很大。
為了維持望遠鏡的穩定，我們會需要遮陽板，例如[韋伯望遠鏡用了一個網球場大小的遮陽板](https://webb.nasa.gov/content/observatory/sunshield.html)

![每一個器材都需要大量時間和金錢來研發和測試](https://i.imgur.com/gx3PK06.png)

這裡有一個要注意的小知識：[*拉格朗日點*](https://zh.wikipedia.org/zh-tw/拉格朗日点)。
很多太空設備都會躲在地球對應太陽的正後方，並利用地球來遮陽，這個位置稱為拉格朗日點（事實上有四個點）。
或者在地球和太陽的中間，讓該設施處於兩中重力的中心點。
...
我們沒辦法使用 Wi-Fi 但我們可以透過電磁束打出二進位資料。
但為了維持穩定性，會希望望遠鏡待在離地球遠一點的地方，這樣他的繞地週期才會常，
...
總而言之，要做一個太空望遠鏡，為了解決上述問題，會需要很多很多很多很多的錢。
除此之外，當需要維修時，又會花費額外的鉅額預算。
...
!!! info "地球各種輻射帶、特殊點的比例圖"
    ...
    6. *月球繞行* 約為 61 個 $R_e$
    7. *拉格朗日點* 約為 236 個 $R_e$
    8. 太陽約為 23,000 個 $R_e$

## 星體

在上一段中，介紹了 *赫羅圖* 中星星的分佈，你會注意到其有一個主要分佈帶從右下至左上，我們稱其為 *主序帶*（Major sequence）。
這代表大部分星星在越接近藍色（越熱）它的發光程度會越高，為什麼？
因為越亮的星星，它會燃燒越多的燃料（核融合中的氫原子），其能造成的溫度也就越高。
但是為什麼燃燒中的星星不會爆炸呢？

這就是因為星星不只在承受燃燒所造成的向外壓力，同時也面對著重力的壓縮。
在這兩個力量的平衡下，星星的大小就能夠維持著，例如現在我們看到的太陽。

> 我們透過觀察核融合的產品：*微中子*，來驗證我們的猜想。

在 *主序帶* 之上，也有一群分布較為密集的區域，我們稱其為巨星分支（Giant branch），就是常聽到的紅巨星的所在位置。
這是因為星星在燃燒的原料（核融合中的氫原子）用盡後，會開始收縮其內部核融合的區域。
當收縮到一定程度後，由於其極高溫高壓，帶動周圍的物質進行新一波的核融合反應（若星體過於龐大，收縮速度快到不足以產生足夠的力對抗，就會形成[黑洞](#_20)）。
...
以下展示幾張圖，皆是來源於[哈伯望遠鏡](https://hubblesite.org/resource-gallery/images)。

|[The Crab Nebula](https://hubblesite.org/contents/media/images/2020/03/4601-Image) | [Hourglass Nebula](https://hubblesite.org/contents/media/images/2019/15/4487-Image) |
| - | - |
| ![紅巨星的噴發](https://i.imgur.com/Je3mYRV.png) | ![紅巨星噴發後形成的星雲](https://i.imgur.com/hMmRQNE.png) |
...
有了星體的知識和太空望遠鏡的建造，我們終於可以觀測到黑洞了。

當星體過於龐大，其因為重力而收縮的力大到沒有任何力能有效對抗其存在時，就會形成黑洞。
黑洞形成時，它的重力會大到影響任何靠近他的物體，當物質繞著黑洞運行，並逐漸靠近時，該物質會開始加速，並和其他物體碰撞產生高溫。
最終當速度越來越快，溫度越來越高，這時，其釋放的高能電磁波（例如 *X射線*）就可以被捕捉，進而觀察出黑洞的存在。
...
![天鵝座X-1 觀測其 *X射線* 所得到的圖像](https://i.imgur.com/ZC2f2e8.png)

首先我們先來推算該黑洞的體積上限。
由於我們觀察到這個黑洞（和其周遭的物體）發射出的 *X射線* 的強度每秒會變動約一百次，這就代表黑洞的物體繞行時間約為百分之一秒。
...
接下來我們看一下他的質量。
我們可以透過繞行黑洞的[一個藍巨星](https://astronomy.com/news/2021/03/cygnus-x-1-the-black-hole-that-started-it-all)來計算簡單牛頓力學並取得質量。假設黑洞質量（M）和藍巨星質量（m）會因為重力而彼此吸引，其力會用來當作繞行的角速度（v）：
...
> 同樣的雙星運行的計算也會比上述的還要複雜很多，這邊也忽略。

之所以有這些結果都是因為我們成功在外太空裝設了望遠鏡。
...
!!! info "時光機器"
    ...
    例如目前（2022）可觀測最遠的星體是 [GN-z11](https://en.wikipedia.org/wiki/GN-z11)，它距離地球 134 億光年，
...
單純以感光能力來說明，以傳統[感光幹板](https://en.wikipedia.org/wiki/Photographic_plate)為例，
感光幹板能處理接收約 1% 的光線，但 CCD 卻可以處理到 80%。
...
能讓特地物質的電子躍遷到可以傳導電子的程度（或稱[導帶](https://en.wikipedia.org/wiki/Valence_and_conduction_bands)）時，
就可以依此來判斷光線的頻率。
當持續照射電磁波，並累積足夠的電子時，我們就可以把這些電子收集並計算數量，最終依此來判斷光線的強弱。

這種物質我們通常會選矽，因為他很便宜又很好印刷出電路。
因為其便利性，所以讓矽擁有很多方面的應用，也因此在民間（非天文學上）進行很多的實驗與嘗試。
越多的關注就又進一步的提升該物質的良率和功能，也就重新回饋給天文學的進展。
這也回到最一開始的問題，天文學有什麼用？

#### 要怎麼收集電子？

用 [MOS](https://en.wikipedia.org/wiki/MOSFET) （Metal–Oxide–Semiconductor）來收集。
其是利用外部電壓（Metal）讓半導體（Semiconductor）中的電子朝著特定方向前進，最終會有個絕緣體（Oxide）擋住電子，並困住他。

![MOS 的架構圖](https://i.imgur.com/SVAgcIt.png)

CCD 中的每一個位元都是一組 MOS，所以 6x6 cm 的 CCD 就有 4000x4000 個位元（$6cm / 15um = 4000$）。
...
透過循序釋放電壓，把電子慢慢累積並統計，這方法慢（6x6 cm 的 CCD 需要約三十秒來判讀）但是精準。

### 紅外線感測

很多東西會發射紅外線，例如冷的東西、被微塵包裹的物體、宇宙邊緣紅移的物體。
然而紅外線的能量不夠讓矽半導體進入[導帶](#_22)，
所以我們需要其他物質，例如 [CMT](https://en.wikipedia.org/wiki/Mercury_cadmium_telluride) ，來捕捉近紅外線。

每種不一樣的頻率的電磁波，很可能就會需要再用不一樣的材質，而這些材質不像矽這麼通用。
所以通常都會又昂貴又精密且可能全球就只有數個團隊在做研究而已。
...
因為 *都普勒效應*，我們可以透過紅移的現象，來暸解這個星體和我們的距離。
...
該星系是目前已知最遠最古老的星系，距離地球約 130 億光年，能夠幫助我們理解宇宙成形之初的樣子。

### 感測器的其他議題

感測器需要注意溫度。

因為物體只要有溫度就會發射電磁波，所以需要讓感測對象（目標星體）之外的環境盡量冷卻，否則會影響，即所謂的熱干擾。
電子的碰撞和物體的晃動都可能讓他突破導帶，並影響成像，我們可以把感測器放在冷卻液，例如液態氦中。

除了 CCD，近期也有研究其他類型的感測器，例如 KIDS（Kinetic Inductance Detection System）。

## 電腦

隨著人們可以觀測的星體越來越多，我們需要的是一個可以 *儲存* 大量紀錄的機制和可以進行繁雜且精密的 *計算*。
電腦在 1950 年代之後，逐漸在天文學中佔有非常重要的一塊，其優勢有：

-   可以直接儲存、運算數位化的感測結果。
-   可以控制望遠鏡，我們在[大氣干擾](#_7)有提到天體的光線進入地球前會晃動。
    ![利用雷射取得目前大氣對光干擾的程度](https://i.imgur.com/adOHd50.png)

    > [CANARY's Laser Launch](https://www.ing.iac.es//PR/press/canary_photo.html)

    透過打出的雷射，計算當前大氣對光線的影響程度，在進行望遠鏡的移動和成像的修正。

-   可以進行複雜且精密的計算，例如我們可以「製造」出數十億個假的星體，並模擬其碰撞、互動等等的機制，來驗證各種假說。
-   可以儲存大量的紀錄，世界各地都有天文學家，透過資料的儲存和傳遞，
  我們可以快速把自己的結果（例如 X 射線的曝光結果）和別人的結果（例如紅外線的曝光結果）進行比對。
...
CCD 中每個位元（MOS）可以儲存 2 bytes（16 bits）的資訊，如果以 [4000 個位元](#_23)來計算，
每張 CCD 出來的影像就會有 32 MB 的大小。
我們再以以一個望遠鏡有多個 CCD 來考慮，正常一張照片約會需要數 GB。
...
假設我們有一百萬個物體（$10^6$），就會需要 $(10^6)^2 = 10^{12}$ 次計算，當數量達到數億個，將讓電腦計算時間拉長到失去意義。
而且這還只是一瞬間的互動，如果要紀錄一百步甚至數億步，這個時間將會拉得更長。

所以我們只能取大約的結果，也就是只計算較有影響力的互動。
通常會讓時間變成 $n\log n$ 這種等級的成長。

總的來說，[大量的儲存](http://www.sdss3.org/index.php)和[高速的運算](http://www.ncsa.illinois.edu/enabling/bluewaters)，讓天文學家通常會利用資料中心來做這件事。
透過遠端去檢視、計算需要的資訊和結果。
並且相關的協定也已經在製作中，例如 [IVOA](https://www.ivoa.net)。
...
我們每晚會紀錄各個星體的位置，並確認其移動週期和移動方向，是繞著太陽轉還是朝著地球走。
...
頻率越高，溫度越高。
換句話說，星體呈現藍色代表它的的溫度會比紅色高，然後統計各個星體的溫度後就可以畫出 *赫羅圖*。

顏色的光譜圖獲得方法在前面的[波長](#_9)有提到。
...
上圖我們可以看到，左邊是高頻（透過 *都普勒效應*，可以知道這側正靠近我們）右邊是低頻（遠離）所以知道該星系正在以逆時針的方向旋轉，
然後再透過前述[測量黑洞質量的手法](#_20)，發現整個星系的質量大於星系中星星總的質量和，所以估計中間有個黑洞。

### 觀測方法

早期牛頓使用三角稜鏡（prism）觀測光的可見光光譜，但在現代天文學中卻是使用繞射光柵（Diffraction grating）。
...
要做到這樣，就需利用光纖（optical fibres），把各個星體的光源都移至相同的陣列中，並同時觀察他們。
...
整個宇宙事實上就是如此，只是向外的力量（暗能量，約佔 70%）大於向內壓縮（暗物質，約佔 25%），所以我們才觀察得到宇宙正在擴張。
...
透過 *都普勒效應*，我們只要知道物體發出的頻率漸漸提高，就可以知道他它正在遠離，問題是我們不可能觀察數十萬年來證明它的變化。
所以我們需要一種星體，不管是在哪邊生成，他的亮度都要一樣，*1a 類超新星* 就是這樣一種星體。
...
因為白矮星吸收旁邊恆星釋放的物質後引發的 *1a 類超新星* 爆炸的質量都差不多是 1.4 倍的太陽質量，
所以他們釋放的能量和亮度也都差不多。

由此觀察不同的 *1a 類超新星*，就可以知道越暗（遠）的 *1a 類超新星* 其頻率越低，代表他們正更快速地遠離我們。
...
透過上述觀察到的事實，論文中就指出宇宙組成的比例為 70% 的暗能量、25% 的暗物質和 5% 的已知物質。

Digital Micro Mirrors

## 總結

有了這些知識，當你在欣賞[韋伯望遠鏡所拍攝的照片](https://www.bbc.com/zhongwen/trad/science-64101354)和理解其運作原理時，是不是更有感呢！
...
*[都普勒效應]: 靠近我們的物體，其頻率會漸漸提高，遠離我的們的物體，則是會降低頻率（可以想像呼嘯而過的汽車）。
