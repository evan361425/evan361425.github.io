...
為了滿足目標，我們要可以讓運行在 PCC 的程式被資安專家下載並進行檢查，
同時也要有機制驗證該程式真的有運行在 PCC 中。
另一方面，所有對 PCC 的請求都應該進行點到點的加密，且使用的金鑰是 PCC 每個節點特有的金鑰。

為了達到前述兩個目的，使用者在對 PCC 進行 LLM 請求前會先被選擇到一個特定節點，
並且之後的一段時間，請求都會被導流到該節點，在初次溝通時會獲取該節點的公鑰和證明（attestation）。
該證明就包含*該節點的特有金鑰*和*運行程式*的簽章，透過 Trust Signing System (TSS) 即可驗證該簽章。

我們先了解[機密運算]中各名詞代表的意思。
...
- 該硬體與預期的記錄完全匹配；
  ...
  - 移動，從出廠時間到機架上位置移動的時間都會；
  - 啟用，稽核確保使用未經篡改的硬體和預期的軟體；
  - 維修，停止使用原因和時間，設備的替換都會被記錄。
  - 是持續的流程而非僅在初始化時執行，並由第三方審計人員隨機抽查
- 憑證內部的節點公鑰即是 DCIK，並透過 [Secure Enclave] 產生；
- 簽核需由 Apple **三個組織的員工各自攜帶被獨立放進保險箱中的防篡改袋中的權杖**後才能執行，
...
??? tip "iBoot 後面是什麼？"
    ...
    其中在 user space 中有個值得注意的初始換任務稱作 darwin-init，他會負責啟動多個 cryptex，
...
同時，任何程式都必須是編譯過的，當程式要進行更動時，就必須要重啟設備，重新跑一次[啟動流程](#如何確保節點運行正確的程式)。
...
但是應用邏輯程式（LLM 或各種商務邏輯）是被 cryptex 納管的，所以修正應用邏輯的程式不需要重啟。
cryptex 是一個獨立的軟體管理工具，用來分發經過簽章和正確性驗證的程式。
