...
聽起來好像不難，就是再開一個資料庫去放這些資料，但是後面我們會提：當我們需要分散式系統去對某種狀態達成一個共識，是非常困難的，很多邊際情況會出現。把該演算法實作出來同時也會需要注意很多小細節（有點像是密碼學上的實作），透過一個開源且被大量使用的軟體（例如 ZooKeeper）來專精於達成這件事是相對安全的。
...
但是這會增加寫入時的效能，因為是在寫入，所以就很可能會發生競賽狀況，需要分散式的交易（也就是共識）。
...
不過這裡我們只會討論在多資料庫因為延遲導致的資料不一致。而且僅談到「延遲」，不會談到任何資料庫無法回應或網路中斷導致的不一致，因為這需要共識演算法來幫我們處理。
...
!!! info "分散式資料庫只能保持最終一致性？"
    ...
    我們也會在最後一章討論除了分散式的交易（共識演算法）之外，我們還有什麼好方法可以不犧牲高一致性呢？
