...
1. 在多線程中代表每項工作都是單一且獨立的，不受其他線程影響（類似於交易中的[隔離性](#_27)）。
...
接收方接收資料速度較慢時，強制降低傳送方的資料傳送速度，也稱為[流控制](#_20)。
...
接收方接收資料速度較慢時，強制降低傳送方的資料傳送速度，也稱為[反壓](#_4)。
...
透過任意關鍵字來搜尋文字，通常具有附加特徵，例如匹配類似的拼寫詞或同義詞。全文索引是一種支援這種查詢的[次級索引](#_44)。
...
用於描述一種操作可以安全地重試執行（類似於[確定性](#_15)），即執行多次的效果和執行一次的效果相同。
...
在[交易](#_55)上下文中，用於描述並行執行交易的互相干擾程度。序列化隔離性具有最強的隔離性，不過其它程度的隔離也通常被使用。
