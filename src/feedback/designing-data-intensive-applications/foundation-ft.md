...
![使用者2明明有新郵件，但是 `unread` 數量沒有正確顯示](unread-count-sync-error)
...
![同時對單一值加一](concurrent-increment-issue)
...
![糗了，新增郵件後卻在更新未讀數量時網路中斷](atomic-update-network-failure)
...
![交易機制的價值](transaction-mechanism-value)
...
![要求越強的一致性，會帶來一些犧牲](consistency-sacrifice-tradeoff)
...
![因為可以覆蓋掉別人還沒寫完的資料，很可能會造成破碎的狀態](dirty-write-fragmented-state)
...
![使用者僅讀交易完成後的資料，卻仍可能讀到交易的不一致性](read-committed-inconsistency)
...
![你只能讀取同一個版本下的資料庫](snapshot-isolation-reading)
...
![透過資料庫的值來判定邏輯運作時，就可能造成過時的判斷](stale-read-logic-error)
...
![互動式交易和貯存程序的比較](interactive-vs-stored-procedure)
...
![交易43 因為被判定讀取到舊資料而被退回](txn-43-stale-read-abort)
...
![交易42 因為被判定寫入資料時會影響 交易43，故而在 交易42 提交後放棄 交易43](txn-42-43-conflict-rollback)
