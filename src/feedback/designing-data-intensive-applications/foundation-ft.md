...
不是每個資料庫都會做重來的機制，有些資料庫為了滿足[高可用性（High Availability）](introduction.md#_14)等目的，會盡可能做自己能做的事（best effort）。
...
但是到了 2010 年左右，[NoSQL](foundation-model.md) 的理念開始崛起。他們提倡的不只是[不同的資料架構](foundation-model.md#_8)，也放棄使用多值（multi-object）的交易（單一值的交易很輕易就能達成，然而多值的交易卻需要付出龐大的代價），也由此，達成高擴增性、高可用性和高效能的資料庫。
...
我們前面提到[實際序列化](#_20)和[兩階段鎖](#_23)分別有低擴增性和低效率性的缺點，然而序列化快照卻修正了這些缺點，進而**可能**成為未來實作序列化一致性時的標準做法。
