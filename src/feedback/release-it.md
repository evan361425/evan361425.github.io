...
> 每一個案例都會有個引用，代表這個案例的出處和頁數（注意，頁數對應的是第二版）。
...
目的端並未告知請求的錯誤是暫時性（例如鎖死）還是永久性（例如輸入格式錯誤），
所以來源端反覆重新發送，加速雙方的消亡。
...
前端使用者錯誤設置的代理者（例如 [Chrome 的優化設定](https://blog.csdn.net/u011474608/article/details/115178691)）會反覆發送請求，
...
-   建置防禦 DDoS 機制；
-   讓前端（SPA 或 SSR）送請求來建立 session，將低非人類請求去建立 session 的可能；
-   艙壁設計原則，避免服務的失能
...
套件管理工具在服務升版過程把預期被關掉的 auto-scaler 啟動，
...
但意外就發生在這些理所當然的事情上，應用程式處理時沒有使用 [pagination](https://itnext.io/the-best-database-pagination-technique-is-530abf2aab51?gi=2ecb66757cea)，
...
當服務的其中一個小服務的外部相依能處理的量變很低時，導致其他服務完全失能。
這是因為連線池的連線是共用的，其外顯特徵包括：頻寬很高、延時很長、資源（CPU/Mem）使用率低。
...
而是要指定 domain 或 IP（`app.example.com:8080`）。
...
-   小服務設定檔用注入（檔案或環境變數）；
    大（多個微）服務可以用專門服務來替代（Consul/ZooKeeper/etcd）。
...
??? example "AWS 的失能案例"
    ...
    *一位授權的管理者根據指南進行操作，在執行一個關閉單一節點的指令時，因為指令的錯誤，
    導致關閉了大部分的節點*，在此我們可以反思幾件事情：
...
身為一個監控系統要怎麼滿足這些東西？
[串流處理](./designing-data-intensive-applications/derived-stream/#_3)。
...
就像[尚未出現的整合資料的服務](./designing-data-intensive-applications/farewell/#_11)一樣，
