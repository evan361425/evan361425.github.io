...
除了應用程式用的飛地外，也有很多系統用的飛地，
這些飛地存在目的通常就是提高飛地的安全性和更完整的保護飛地的整個生命週期。
...
還有一個和機密運算有一些重疊的就是 Trust Platform Module (TPM)。
TPM 是一個獨立的硬體安全模組，可作為獨立晶片或嵌入主板，通過作業系統整合使用，其目的有三個：

- 硬體隔離密鑰：密鑰存放於獨立模組內，防止軟體竊取與惡意存取；
- 可信啟動與完整性測量：從 BIOS 到 OS 建立完整的信任鏈，利用硬體根信任確保設備狀態是正確的；
- 硬體加解密：提供專用運算單元，提升安全性及效能，同時支持密鑰封存與遠端證明。

簡單來說，TPM 是硬體信任根，確保設備與系統的可信狀態，提供密鑰管理、完整性驗證，
Intel SGX 是機密計算技術，目的在於保護應用程式執行中的數據，即使 OS 或攻擊者控制系統，也無法存取數據。
兩者可互補，例如使用 TPM 進行遠端證明，確保 SGX 環境未被破壞。
