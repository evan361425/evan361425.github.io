...
當有了共識是新的專案預設採用後，我們面對舊專案又該怎麼做呢？
有兩個主要方向，一個是有規劃地逐步全面汰換，
另一個如果沒有足夠資源可以進行全面汰換，就可能需要採用共存或小團隊內的導入。

如果是逐步汰換，流程大致如下：

1. 確保上層的支持；
2. 建立專責團隊，負責規劃和建立共用工具；
...
6. 預設且建議使用新語言，並且幾乎沒有例外狀況，專責團隊可以進行重整。
...
記憶體安全的優勢在於能夠降低長期產品的安全相關成本。
透過從源頭減少記憶體安全漏洞，就能夠將漏洞成本轉移到了軟體開發生命週期的早期階段，這將大幅降低成本。
請理解導入的必要性、優勢還有困境，並根據需求選擇可行的方式進行，祝你好運。
